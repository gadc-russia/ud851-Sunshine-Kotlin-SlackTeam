...
internal object TestUtilities {
    ...
    val testContentObserver: TestContentObserver by lazy { TestContentObserver.getTestContentObserver() }
    ...
    fun createTestWeatherContentValues(): ContentValues {
        ...
        testWeatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_DATE, DATE_NORMALIZED)
        testWeatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_WIND_DIR, 1.1)
        testWeatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_HUMIDITY, 1.2)
        testWeatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_PRESSURE, 1.3)
        testWeatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_MAX, 75)
        testWeatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_MIN, 65)
        testWeatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_WIND_SPEED, 5.5)
        testWeatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_WEATHER_ID, 321)
    ...
    fun createBulkInsertTestWeatherValues(): Array<ContentValues> {
        val bulkTestWeatherValues = ArrayList<ContentValues>(BULK_INSERT_RECORDS_TO_INSERT)
        ...
        for (i in 0 until BULK_INSERT_RECORDS_TO_INSERT) {
            ...
            weatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_DATE, normalizedTestDate)
            weatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_WIND_DIR, 1.1)
            weatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_HUMIDITY, 1.2 + 0.01 * i.toFloat())
            weatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_PRESSURE, 1.3 - 0.01 * i.toFloat())
            weatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_MAX, 75 + i)
            weatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_MIN, 65 - i)
            weatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_WIND_SPEED, 5.5 + 0.2 * i.toFloat())
            weatherValues.put(TestSunshineDatabase.REFLECTED_COLUMN_WEATHER_ID, 321)

            bulkTestWeatherValues[i] = weatherValues
        }

        return bulkTestWeatherValues.toTypedArray()
    }
    ...
    fun studentReadableClassNotFound(e: ClassNotFoundException): String {
        ...
        var simpleClassNameThatIsMissing = message.substring(indexBeforeSimpleClassName + 1)
